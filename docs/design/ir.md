...
The IR contract for Continuum. The canonical machine-readable schema lives at
[`schema/cir.fbs`](../../schema/cir.fbs); this document describes the runtime
wire format that `Graph::serialize()` emits.

## Binary envelope

`Graph::serialize()` emits a compact binary envelope.
