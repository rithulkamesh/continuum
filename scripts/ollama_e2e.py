...
def check(name: str) -> Callable[[Callable[..., dict[str, Any]]], Callable[..., None]]:
    def wrap(fn: Callable[..., dict[str, Any]]) -> Callable[..., None]:
        def run(*args: Any) -> None:
            ...
            try:
                detail = fn(*args)
                RESULTS.append({"check": name, "ok": True, "seconds": round(time.perf_counter() - t0, 2), **detail})
...
def durable_agent(base: str, model: str) -> dict[str, Any]:
    ...
    try:
        ...
        prompts = ["Name a primary color.", "Name a fruit of that color.", "Name a dish with that fruit."]
        rec = DurableAgent()
        assert rec.backend == "vllm", rec.backend
        rec.begin(prompts, model_id=f"vllm/{model}", max_tokens=24)
        ckpt = rec.run_until_step(0)

        a, b = DurableAgent(), DurableAgent()
        a.resume_from(ckpt)
        forked = DurableAgent.fork(ckpt, rec.prompt_node_ids[1], "Name a vegetable of that color.")
        b.resume_from(forked)
        sa, sb = a.step_outputs(), b.step_outputs()
        assert all(isinstance(x, str) and x.strip() for x in sa + sb), (sa, sb)
        assert sa[0] == sb[0], "step 1 must replay from the checkpoint, not regenerate"
        assert sa[1] != sb[1], "the edited step must diverge"
        return {"steps_a": [x[:60] for x in sa], "steps_b": [x[:60] for x in sb]}
    finally:
...
def proxy(base: str, model: str) -> dict[str, Any]:
    ...
    try:
        ...
        first = client.chat.completions.with_raw_response.create(model=model, messages=msgs, temperature=0)
        t0 = time.perf_counter()
        second = client.chat.completions.with_raw_response.create(model=model, messages=msgs, temperature=0)
        ...
        streamed = "".join(
            ...
            for c in client.chat.completions.create(model=model, messages=smsgs, temperature=0, stream=True)
            if c.choices
        )
        replay = "".join(
            c.choices[0].delta.content or ""
        ...
        return {"answer": text[:80], "hit_latency_ms": round(hit_ms, 1), "requests": metrics["requests"]}
    finally:
        p.close()
...
def semantic(base: str, model: str, embed_model: str) -> dict[str, Any]:
    ...
    sims = {q: SemanticCacheIndex.cosine_similarity(vecs[cached], vecs[q]) for q in (rewording, near_miss)}
    ...
    served_rewording = idx.lookup(vecs[rewording], "m", embedder_id=emb.identity(), query_prompt=rewording)
    served_near_miss = idx.lookup(vecs[near_miss], "m", embedder_id=emb.identity(), query_prompt=near_miss)
...
def prefix(base: str, model: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"cold_p50_ms": round(statistics.median(cold), 1), "warm_p50_ms": round(statistics.median(warm), 1),
...
def main() -> int:
    ...
    if args.json:
        with open(args.json, "w") as f:
            json.dump({"base_url": base, "model": args.model, "embed_model": args.embed_model,
