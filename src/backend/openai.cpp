...
std::vector<std::int32_t> ExtractPromptTokens(const std::vector<continuum::Value>& inputs, int salt) {
  ...
  for (unsigned char c : prompt) {
    out.push_back((static_cast<int>(c) + salt) % 10007);
  }
  return out;
}
...
continuum::TokensValue GenerateDeterministicOutput(
  ...
  for (std::int32_t i = 0; i < max_tokens; ++i) {
    ...
    out.ids.push_back((base + salt + i) % 10007);
  }
  return out;
}
...
BackendRunResult OpenAIBackend::run_with_cache(
  ...
  BackendRunResult r;
  r.output = GenerateDeterministicOutput(prompt_tokens, max_tokens, 17);
